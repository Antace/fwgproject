...
while ($row = mysqli_fetch_array($result)) {
  echo "<tr>";
    ...
    if($row["production_status"]==0){
    echo "<td align=center><a href='production.php?act=view&ID=$row[production_id]' class='btn btn-info btn-xs'><i class='fas fa-folder-open'></i></a>
    <a href='production.php?act=production_cancel&ID=$row[production_id]' class='btn btn-danger btn-xs'><i class='fas fa-trash'></i></a></td> ";
    }else if($row["production_status"]>0){
      echo "<td align=center><a href='production.php?act=view&ID=$row[production_id]' class='btn btn-info btn-xs'><i class='fas fa-folder-open'></i></a>
    <a href='production.php?act=production_cancel&ID=$row[production_id]' class='btn btn-danger btn-xs disabled' ><i class='fas fa-trash'></i></a></td> ";
    }
	echo "</tr>";
  }
  ?>
