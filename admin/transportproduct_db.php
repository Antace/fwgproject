...
//*** Check Year ***//
$strSQL = "SELECT * FROM tb_prefixtransport WHERE 1 ";
...
if($objResult["val"] == $date1)
{
	...
	//*** Update Next Seq ***//
	$strSQL = "UPDATE tb_prefixtransport SET mval = '".date("m")."' ,seq= seq+1 ";
	$objQuery = mysqli_query($con,$strSQL) or die ("Error Query [".$strSQL."]");
}
else  //*** Check val != year now ***// (แต่ถ้าไม่ใช่ ปีปัจจุบัน จะทำการ ขึ้นปีใหม่ เช่น ถ้าปีนี้ 66 แต่ ค่า val = 65 จะเริ่มนับใหม่ เป็นปีปัจจุบันแทน)
{
	...
	//*** Update New Seq ***//
	$strSQL = "UPDATE tb_prefixtransport SET val = '".$date1."' , mval='".date("m")."', seq = '1' ";
	$objQuery = mysqli_query($con,$strSQL) or die ("Error Query [".$strSQL."]");
}
...
$total = mysqli_real_escape_string($con,$_POST[ "transport_total" ]); //ราคารวมทั้ง order
$discount = mysqli_real_escape_string($con,$_POST["transport_discount"]);
$vat = mysqli_real_escape_string($con,$_POST[ "transport_vat" ]);
$stotal = mysqli_real_escape_string($con,$_POST[ "transport_stotal" ]);
...
$sql1 = "INSERT INTO tb_transport
VALUES

($strNextSeq,'$transport_date','$customer_name','$department_name','$total','$discount','$vat','$stotal','$username','$transport_dt')";
...
$sql2 = "SELECT MAX(transport_id) as transport_id 
	FROM tb_transport
	WHERE customer_name='$customer_name' ";
$query2 = mysqli_query($con, $sql2) or die("Error in query: $sql2" . mysqli_error($con,$sql2));
$row = mysqli_fetch_array($query2);
$transport_id = $row["transport_id"]; // order id ล่าสุดที่อยู่ในตาราง order_head
...
foreach ($_SESSION['transport'] as $product_id => $qty) {
  ...
  $pricetotal = $transport_price * $qty;
  $count = mysqli_num_rows($query3);



  $sql4 = "INSERT INTO tb_transportlist VALUES(null, $transport_id,'$transport_po' ,$product_id, $qty,$transport_price)";
...
if ($query1 && $query4) {
  ...
  foreach ($_SESSION['transport'] as $product_id) {
    //unset($_SESSION['transport'][$p_id]);
